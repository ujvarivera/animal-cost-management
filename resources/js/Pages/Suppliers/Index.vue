<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            Beszállítók
            <ButtonLink v-if="permissions.manage" :href="route('suppliers.create')" class="bg-purple-800 hover:bg-purple-700">Új beszállító</ButtonLink>
        </template>

        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 border-b border-gray-200">

                <DataTable v-model:filters="filters" :value="suppliers" paginator :rows="5" 
                    :rowsPerPageOptions="[5, 10, 20, 50]" sortMode="multiple" removableSort showGridlines 
                    tableStyle="min-width: 50rem" :globalFilterFields="['name', 'tax_number', 'registration_number', 'phone_number']">
                    <template #header>
                    ...
                    <template #empty> Nem található beszállító. </template>
                    ...
                    <Column header="Törlés" v-if="permissions.manage">
                        <template #body="vet">
                            <ButtonLink method="delete" :href="route('suppliers.destroy', vet.data)" class="bg-red-400 hover:bg-red-500">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
